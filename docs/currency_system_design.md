...
为了契合 **LustfulBoy** 的品牌调性，我们将采用更具“欲望”色彩的设定。

*   **代币名称**: **精粹 (Essence)**
*   **代币符号**: `💧` (代表体液、能量结晶)
*   **核心逻辑**: 
    *   用户通过活跃（签到、聊天、互动）向 Bot **"注入"** 能量，提取出的结晶即为 **"精粹"**。
...
*   **状态显示**: 
    *   未签到时：按钮亮起，文案为 `📅 每日注入`。
    *   已签到时：按钮变灰或文案变为 `✅ 今日已注入`，并在面板下方显示 `💧 精粹: 120`。
*   **交互流程**:
    1.  用户点击 `[📅 每日注入]`。
    2.  Bot 弹窗 (Alert) 提示：
        > "注入成功！💓
