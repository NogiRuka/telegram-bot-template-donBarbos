...
### 1. `user_extend` (用户扩展表)
复用已有的 `user_extend` 表，新增精粹系统相关字段。
...
### 1. `essence_rules` (精粹规则表)
存储各种行为的奖惩数值，支持动态调整。

| 字段名 | 类型 | 属性 | 说明 |
| :--- | :--- | :--- | :--- |
| `id` | Integer | PK, Auto | 规则ID |
| `rule_key` | Varchar(64) | Unique | 规则键名 (如 `checkin_base`, `checkin_bonus_rate`) |
...
### 2. `essence_products` (精粹商品表)
存储商店可售卖的商品信息，支持上下架、有效期管理以及**显示权限控制**。
...
| `visible_roles` | JSON | Nullable | **显示权限**: 允许看到该商品的角色列表 (如 `["user", "admin"]`)，空则全员可见 |
