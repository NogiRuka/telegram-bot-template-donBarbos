...
### 3.3 user_main_image_prefs（用户主图偏好）
| 字段 | 类型 | 说明 |
|---|---|---|
| user_id | BigInt, PK | 用户ID |
| display_mode | Varchar(16) | 展示模式：sfw/nsfw/random |
...
索引建议：
- idx_user_prefs_nsfw (nsfw_unlocked)

## 4. 权限与开关
- 全局开关：在配置表中加入 `main_image.nsfw_enabled`（Boolean），管理员面板可开关 NSFW。
- 用户解锁：商店商品“解锁🔞主图”购买后，写入 `user_main_image_prefs.nsfw_unlocked = True`。
- 双重判定：仅当 `nsfw_unlocked == True` 且 `main_image.nsfw_enabled == True` 时，用户可选择 NSFW 或随机包含 NSFW。
...
- 购买效果：
  - 在 `CurrencyService._handle_product_effect` 中对 `action_type == "main_image_unlock_nsfw"`：
    - 写入/创建 `user_main_image_prefs`，将 `nsfw_unlocked = True`（幂等）。
...
- 记录结果：更新 `user_main_image_prefs.last_image_id`。
...
## 10. 开发计划（实施步骤）
1. 建表：`main_images`、`main_image_schedule`、`user_main_image_prefs`。
...
- SQLAlchemy（异步）：`pip install sqlalchemy aiosqlite`（如使用 SQLite）
