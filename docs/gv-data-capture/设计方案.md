...
- 目前以电影为主。
- Emby 可通过公网地址访问。
- 首个数据源为日韩分类的 `www.ck-download.com`。关键词搜索通过 `POST /product/search` 提交 `kw`、`kw_opt=1`、`only_nm=0`，详情页为 `/product/detail/{numeric_id}`；当前解析番号加标题、原标题、排序名、上架日、简介、时长、厂家、播放内容、模型类型、出演模型、标签、DVD 信息和 `_1.jpg` 主图，并兼容单品页与套装页两种图片区域结构。
