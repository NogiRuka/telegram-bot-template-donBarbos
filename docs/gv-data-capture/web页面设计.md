...
- 主工作区中部：候选列表，展示来源、番号、标题、年份、置信度和详情状态。
- 主工作区下部：字段对比表、字段勾选、覆盖模式、确认写回。
- 操作抽屉：本次请求状态、错误摘要和最近操作，不展示 API Key、Cookie 或完整外部请求头。
...
- `CandidateList`：候选单选、置信度、精确番号标识、来源链接。
...
搜索期间禁用重复提交；无候选时保留关键词供修改；候选缺封面时显示“来源未提供公开封面”，不显示占位海报；写回失败时保留用户勾选状态以便重试。
...
搜索响应应返回 `extracted_product_number`、`category`、候选列表和数据源错误；单个数据源失败不伪装为空结果。
...
- 限制并发和访问频率；第一阶段单次搜索最多补抓 `limit` 个详情，默认超时 15 秒，不对推荐项目全量抓取。
- `ck-download` 首页条款说明内容仅限私用查看，二次利用需授权。投入正式使用前必须确认抓取、展示及写回用途获得授权，并遵守站点 robots 与频率要求。
- 适配器只获取公开文字元数据，不绕过登录、年龄确认、验证码或付费边界，不下载图片、视频或样片。
- 图片版权独立于文字元数据；无明确公开产品主图或无授权时不抓取、不缓存、不写回，严禁用 logo、banner 或猜测 URL 代替。
...
| 6 | 搜索返回重复详情链接 | 候选按 `source_id` 去重，只显示一次 |
| 7 | ck-download 候选没有公开主图 | 显示“来源未提供公开封面”，不出现假封面和封面写入选项 |
| 8 | 当前简介已有值且使用仅填空模式 | 简介默认不覆盖，确认清单不包含该字段 |
| 9 | 切换为覆盖模式并勾选年份 | 对话框明确显示旧值、新值，确认后只写入所选字段 |
| 10 | 数据源超时或页面结构变化 | 显示具体错误，不修改 Emby，保留 Item、关键词和字段选择 |
