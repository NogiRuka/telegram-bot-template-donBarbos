...
def main():
    ...
    print(f"✅ 生成功能注册: {registration_file}")
    
    # 输出使用说明
    print("\n📋 下一步操作:")
    print(f"1. 编辑处理器文件: {handler_file}")
    print(f"2. 在启动时导入注册: from bot.features.registrations.{args.name.replace('.', '_')} import *")
    print(f"3. 运行程序测试功能")
    
    print(f"\n🎯 功能已注册，回调数据为: {args.name.replace('.', ':')}")
