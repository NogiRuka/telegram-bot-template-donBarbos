...
class MainImageService:
    ...
    async def get_user_prefs(session: AsyncSession, user_id: int) -> dict[str, Any]:
        ...
        if not ext:
            return {"display_mode": "sfw", "nsfw_unlocked": False, "last_image_id": None}
        return {
            "display_mode": ext.display_mode or "sfw",
    ...
    async def select_main_image(session: AsyncSession, user_id: int) -> MainImageModel | None:
        ...
        nsfw_enabled = bool(await get_config(session, "admin.main_image.nsfw_enabled") or False)

        # 1. 节日主图优先
        sched_stmt = (
            select(MainImageScheduleModel)
            .where(MainImageScheduleModel.start_time <= now, MainImageScheduleModel.end_time >= now)
        ...
        if schedule:
            ...
            if img:
                # 检查nsfw约束
                if img.is_nsfw and not (nsfw_enabled and prefs["nsfw_unlocked"] and prefs["display_mode"] in ("nsfw", "random")):
                    pass
                else:
                    return img
        ...
        allow_nsfw = nsfw_enabled and prefs["nsfw_unlocked"] and mode in ("nsfw", "random")
        ...
        if mode == "sfw":
            ...
            if not allow_nsfw:
                cond_stmt = cond_stmt.where(MainImageModel.is_nsfw.is_(False))
        cond_stmt = cond_stmt.order_by(MainImageModel.id.desc()).limit(1)
