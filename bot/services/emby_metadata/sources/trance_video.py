...
class TranceVideoSource(MetadataSource):
    ...
    async def search(self, keyword: str, limit: int=10) -> list[MetadataSearchResult]: return TranceVideoParser.parse_search_results(await self._request(f"/product/result?keyword={quote_plus(keyword.strip())}"), limit)
