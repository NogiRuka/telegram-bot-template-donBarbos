...
async def render_category_list(session: AsyncSession, main_msg: MainMessageService, user_id: int):
    """渲染分类列表"""
...
async def list_categories(callback: CallbackQuery, session: AsyncSession, main_msg: MainMessageService):
    """显示分类列表"""
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def add_category_start(callback: CallbackQuery, state: FSMContext, main_msg: MainMessageService):
    """开始添加分类"""
    text = "*➕ 添加分类*\n\n请输入新分类的名称："
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def add_category_process(message: Message, state: FSMContext, session: AsyncSession, main_msg: MainMessageService):
    """处理添加分类"""
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def view_category(callback: CallbackQuery, session: AsyncSession, main_msg: MainMessageService):
    """查看分类详情"""
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def edit_category_start(callback: CallbackQuery, state: FSMContext, main_msg: MainMessageService):
    """开始编辑分类名称"""
    cat_id = int(callback.data.split(":")[-1])
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def edit_category_process(message: Message, state: FSMContext, session: AsyncSession, main_msg: MainMessageService):
    """处理编辑分类名称"""
    await main_msg.delete_input(message)
    name = message.text.strip()
    if not name:
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def toggle_category(callback: CallbackQuery, session: AsyncSession, main_msg: MainMessageService):
    """切换分类状态"""
    cat_id = int(callback.data.split(":")[-1])
    stmt = select(QuizCategoryModel).where(QuizCategoryModel.id == cat_id)
    cat = (await session.execute(stmt)).scalar_one_or_none()
...
@require_admin_feature(KEY_ADMIN_QUIZ)
async def delete_category(callback: CallbackQuery, session: AsyncSession, main_msg: MainMessageService):
    """删除分类"""
    cat_id = int(callback.data.split(":")[-1])
