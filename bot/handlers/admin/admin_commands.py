...
async def cmd_submission_review(message: Message, command: CommandObject, session: AsyncSession) -> None:
    ...
    - 命令格式: /sr <submission_id> <a/r> <notif_id> <comment...>
      ...
      - notif_id: 关联的通知ID（LibraryNewNotificationModel.id）
      - comment: 审批留言（可选，支持空格）
    ...
    try:
        ...
        submission_id_str, action_str, notif_id_str = parts[0], parts[1], parts[2]
        ...
        # 追加投稿者ID到通知 target_user_id（去重并保持逗号分隔）
        try:
        ...
        try:
            ...
            base_text = (
                f"{result_text}，您的{type_text} *{submission.title}* 已处理。\n"
            )
            if comment:
