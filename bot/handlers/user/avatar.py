...
from bot.keyboards.inline.constants import ACCOUNT_CENTER_LABEL, USER_AVATAR_CALLBACK_DATA
...
async def user_avatar(
    ...
    if not has_ticket:
        await callback.answer(
            "🔴 您尚未购买【修改头像】资格，请前往精粹商店购买。",
            show_alert=True
        )
        return

    # 获取商品信息用于展示（可选）
    await CurrencyService.get_product_by_action(session, "emby_image")
...
async def handle_avatar_photo(
    ...
    try:
        ...
        if not has_ticket:
             await message.answer("🔴 您没有修改头像的资格，请先购买")
             await state.clear()
