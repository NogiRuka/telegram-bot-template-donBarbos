...
from sqlalchemy import String, Text, Index, BigInteger, Enum as SQLEnum, Integer, Boolean
...
class GroupConfigModel(Base, BasicAuditMixin):
    ...
    def is_in_save_period(self, check_time: datetime.datetime | None = None) -> bool:
        ...
        # 检查结束时间
        if self.save_end_date and check_time > self.save_end_date:
            return False
    ...
    def should_save_message(self, message_type: str, is_forwarded: bool = False, 
        ...
        if self.message_save_mode == MessageSaveMode.ALL:
            return True
        elif self.message_save_mode == MessageSaveMode.TEXT_ONLY:
            return message_type == "text" and self.save_text_messages
        elif self.message_save_mode == MessageSaveMode.MEDIA_ONLY:
            return message_type != "text" and self.save_media_messages
        elif self.message_save_mode == MessageSaveMode.IMPORTANT_ONLY:
