...
class MessageModel(Base, BasicAuditMixin):
    ...
    id: Mapped[int] = mapped_column(
        Integer, primary_key=True, autoincrement=True, comment="消息记录ID，自增主键，唯一标识一条消息记录"
    )

    # ==================== Telegram标识字段 ====================

    message_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, index=True, comment="Telegram消息ID，必填字段，Telegram平台的原始消息标识符"
    )

    user_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, index=True, comment="发送用户ID，必填字段，消息发送者的Telegram用户ID"
    )

    chat_id: Mapped[int] = mapped_column(
        BigInteger, nullable=False, index=True, comment="聊天ID，必填字段，消息所在聊天的Telegram聊天ID"
    )
    ...
    text_content: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="文本内容，可选字段，存储消息的文本内容，对于长消息会截取前1000字符"
    )

    caption: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="媒体说明，可选字段，存储图片、视频等媒体消息的说明文字"
    )

    entities: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="消息实体，可选字段，存储消息格式化信息的JSON字符串（链接、提及、粗体等）"
    )

    caption_entities: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="媒体说明实体，可选字段，存储媒体说明格式化信息的JSON字符串"
    )

    # ==================== 文件信息字段 ====================

    file_id: Mapped[str | None] = mapped_column(
        String(255), nullable=True, index=True, comment="文件ID，可选字段，Telegram文件的唯一标识符，用于媒体消息"
    )

    file_unique_id: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="文件唯一ID，可选字段，Telegram文件的永久唯一标识符"
    )

    file_size: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="文件大小，可选字段，文件的字节大小，用于存储统计"
    )

    file_name: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="文件名称，可选字段，原始文件的名称"
    )

    mime_type: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="MIME类型，可选字段，文件的媒体类型标识"
    )

    # ==================== 消息关系字段 ====================

    is_forwarded: Mapped[bool] = mapped_column(
        default=False, index=True, comment="是否转发，默认False，True表示此消息是从其他聊天转发而来"
    )

    forward_from_user_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="转发来源用户ID，可选字段，原始消息发送者的用户ID"
    )

    forward_from_chat_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="转发来源聊天ID，可选字段，原始消息所在聊天的ID"
    )

    forward_from_message_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="转发来源消息ID，可选字段，原始消息的消息ID"
    )

    is_reply: Mapped[bool] = mapped_column(
        default=False, index=True, comment="是否回复，默认False，True表示此消息是对其他消息的回复"
    )

    reply_to_message_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, index=True, comment="回复目标消息ID，可选字段，被回复消息的Telegram消息ID"
    )

    reply_to_user_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="回复目标用户ID，可选字段，被回复消息的发送者用户ID"
    )
    ...
    character_count: Mapped[int] = mapped_column(default=0, comment="字符数量，默认0，消息文本内容的字符数统计")

    word_count: Mapped[int] = mapped_column(default=0, comment="单词数量，默认0，消息文本内容的单词数统计")

    # ==================== 元数据字段 ====================

    language_code: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="语言代码，可选字段，检测到的消息文本语言代码"
    )

    sentiment_score: Mapped[float | None] = mapped_column(
        nullable=True, comment="情感分数，可选字段，消息情感分析的分数，范围-1到1"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_messages_type_created", "message_type", "created_at"
        ),  # 消息类型时间索引，用于按消息类型进行统计分析
        Index(
            "idx_messages_telegram_unique", "message_id", "chat_id", unique=True
        ),  # Telegram消息唯一索引，确保同一聊天中的消息ID唯一
        ...
        Index(
            "idx_messages_user_chat", "user_id", "chat_id", "created_at"
        ),  # 用户聊天组合索引，用于查询用户在特定聊天中的消息
        Index(
            "idx_messages_lang_sentiment", "language_code", "sentiment_score"
        ),  # 语言情感索引，用于多语言消息分析和情感统计
    )
