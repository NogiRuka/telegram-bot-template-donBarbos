...
async def send_group_photo_notification(
    bot: Bot,
    photo_file_id: str,
    user_info: dict[str, str],
    reason: str,
    ...
    📖 FullName @Username Reason
    """
    logger.info(f"尝试发送群组图片通知: group={settings.OWNER_MSG_GROUP}, user_info={user_info}")
    if not bot or not settings.OWNER_MSG_GROUP or not user_info or not photo_file_id:
        return
    ...
    try:
        ...
        await bot.send_photo(
            chat_id=settings.OWNER_MSG_GROUP, 
            photo=photo_file_id,
            caption=msg_text, 
            parse_mode="MarkdownV2"
        )
        logger.info(f"群组图片通知已发送至 {settings.OWNER_MSG_GROUP}")
    except Exception as e:
        logger.error(f"发送群组图片通知失败: {e}")
        # 如果图片发送失败，回退到文本通知
