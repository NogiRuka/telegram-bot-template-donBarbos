...
function CompactGroupedResultPanel({ groups, activeId, selectedResult, setActiveId, selectCandidate, clearResults }: { groups: ResultGroup[]; activeId?: string; selectedResult: string | null; setActiveId: (id: string) => void; selectCandidate: (result: MetadataSearchResult, resultOwnerId?: string) => void; clearResults: () => void }) {
  return <div className='flex min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border bg-white'><div className='flex justify-between border-b px-3 py-2'><b className='text-sm'>搜索结果</b><Button size='icon' variant='ghost' onClick={clearResults}><X className='size-4' /></Button></div><div className='min-h-0 flex-1 space-y-2 overflow-y-auto p-2'>{groups.length ? groups.map((group) => <details key={group.item.notification_id} open={group.item.notification_id === activeId} className={`rounded-lg border ${group.item.notification_id === activeId ? 'border-blue-400 bg-blue-50/40' : 'bg-white'}`} onToggle={(event) => { if ((event.currentTarget as HTMLDetailsElement).open) setActiveId(group.item.notification_id) }}><summary className='cursor-pointer list-none px-2 py-1.5 text-xs font-semibold'>{group.item.item_id} {group.item.item_name}<span className='ml-1 font-normal text-slate-500'>（{group.results.length}）</span></summary><div className='space-y-2 border-t p-2'>{group.results.length ? group.results.map((result) => <div key={`${group.item.notification_id}-${result.source}-${result.source_id}`} className='grid grid-cols-[minmax(140px,42%)_minmax(0,1fr)] gap-2 rounded border bg-white p-2'><div className='flex min-h-32 items-center justify-center overflow-hidden rounded bg-slate-100'>{result.image_urls[0] ? <img src={apiClient.metadataImageUrl(result.image_urls[0], result.detail_url)} alt='' className='size-full max-h-44 object-contain' /> : <Database className='size-7 text-slate-400' />}</div><div className='min-w-0'><b className='line-clamp-3 text-xs leading-4'>{result.title}</b><div className='mt-1 flex flex-wrap items-center gap-1 text-[11px]'><span className='font-semibold text-emerald-700'>{result.price_yen ? `¥${result.price_yen.toLocaleString()}` : '价格未知'}</span>{result.release_date && <span className='text-slate-500'>{result.release_date}</span>}</div><div className='mt-1 flex flex-wrap gap-1'>{result.statuses.length ? result.statuses.map((status) => <span key={status} className='rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-600'>{status}</span>) : <span className='text-[10px] text-slate-400'>无状态</span>}</div><Button size='sm' className='mt-2' variant={selectedResult === result.source_id && group.item.notification_id === activeId ? 'default' : 'outline'} onClick={() => { setActiveId(group.item.notification_id); selectCandidate(result, group.item.notification_id) }}>{selectedResult === result.source_id && group.item.notification_id === activeId ? '已选择' : '选择并抓取'} <ExternalLink className='ml-1 size-3' /></Button></div></div>) : <p className='p-2 text-sm text-slate-500'>该项目没有匹配结果。</p>}</div></details>) : <Empty title='尚未加载搜索结果' text='勾选左侧项目后，点击“批量搜索”。' />}</div></div>
}
