  ...
  return <main className='flex min-h-screen flex-col bg-slate-50 text-slate-800'>
    ...
    <section className='grid min-h-0 flex-1 grid-cols-[minmax(390px,1.05fr)_minmax(340px,.9fr)_minmax(460px,1.2fr)] gap-2 px-5 py-2'>
      <div className='flex min-h-0 flex-col overflow-hidden rounded-lg border bg-white'>
        ...
        <div className='flex-1 overflow-auto'>{queueQuery.isLoading ? <Loading /> : visibleItems.map((item) => { const itemNeedsChoice = (resultsByItem[item.notification_id]?.length ?? 0) > 1; return <button type='button' key={item.notification_id} onClick={() => { setActiveId(item.notification_id); setCandidate(null); setSelectedResult(null) }} className={`grid w-full grid-cols-[28px_1.2fr_1.1fr_90px_55px] items-center gap-2 border-b px-3 py-3 text-left text-sm hover:bg-blue-50 ${active?.notification_id === item.notification_id ? 'bg-blue-50/70' : ''} ${itemNeedsChoice ? 'border-l-4 border-l-amber-400 bg-amber-50/40' : ''}`}>
          <Checkbox checked={selectedIds.includes(item.notification_id)} onClick={(event) => event.stopPropagation()} onCheckedChange={() => toggle(item.notification_id)} /><div><b className='block text-xs text-slate-500'>#{item.item_id}</b><span className='line-clamp-2 font-medium'>{item.item_name}</span><small className='block truncate text-slate-400'>{item.path}</small></div><Input className='h-8' defaultValue={item.search_keyword ?? item.item_name} onClick={(event) => event.stopPropagation()} /><div className='text-xs'><span className='block'>{item.category_label}</span><span className='text-slate-500'>{item.source}</span></div>{itemNeedsChoice ? <span className='rounded bg-amber-100 px-1.5 py-1 text-xs text-amber-700'>待选择</span> : <Status value={item.status} />}</button> })}</div>
