...
function MetadataEditorPanel({ candidate, beforeItem, setCandidate, writeback }: { candidate: MetadataCandidate | null; beforeItem: Record<string, unknown>; fieldSelection?: string[]; setFieldSelection?: (value: (current: string[]) => string[]) => void; setCandidate: (value: (current: MetadataCandidate | null) => MetadataCandidate | null) => void; writeback: () => void }) {
...
function TagEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const values = value.split(/[、,，]/).map((item) => item.trim()).filter(Boolean)
  return <div className='flex min-h-8 flex-wrap content-start gap-1 rounded border bg-slate-50 p-2'>{values.length ? values.map((tag) => <span key={tag} className='inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-xs text-blue-700'>{tag}<button type='button' className='text-blue-500 hover:text-red-600' onClick={() => onChange(values.filter((item) => item !== tag).join('、'))}><X className='size-3' /></button></span>) : <span className='text-xs text-slate-400'>暂无标签</span>}</div>
}
...
function FieldRow({ field, label, candidate, beforeItem, checked, onCheck, hideCheckbox = false, onChange }: { field: FieldKey; label: string; candidate: MetadataCandidate; beforeItem: Record<string, unknown>; checked?: boolean; onCheck?: () => void; hideCheckbox?: boolean; onChange: (value: string) => void }) {
  ...
  const row = <div className={`grid ${gridClass} items-start gap-2 py-2`}>{!hideCheckbox && <Checkbox checked={checked} onCheckedChange={onCheck} />}<label className='pt-2 text-sm'>{label}</label><div className='h-20 max-h-20 overflow-hidden break-words whitespace-pre-wrap text-sm text-slate-500'>{field === 'ExternalIds' ? <ExternalIdsView value={current} /> : collectionFields.has(field) ? <ValueChips value={currentText} /> : currentText || '—'}</div><div className='h-20 max-h-20 min-w-0 overflow-hidden'>{field === 'Overview' ? <textarea className='h-20 max-h-20 w-full resize-y overflow-hidden rounded border px-2 py-1 text-sm' value={value} onChange={(event) => onChange(event.target.value)} /> : field === 'ExternalIds' ? <ExternalIdsEditor value={value} onChange={onChange} /> : field === 'Tags' ? <div className='h-20 overflow-hidden'><TagEditor value={value} onChange={onChange} /></div> : field === 'OfficialRating' || field === 'CustomRating' ? <Select value={value || '__empty__'} onValueChange={(next) => onChange(next === '__empty__' ? '' : next)}><SelectTrigger className='h-20'><SelectValue placeholder='选择评分' /></SelectTrigger><SelectContent>{ratingOptions.map((option) => <SelectItem key={option || '__empty__'} value={option || '__empty__'}>{option || '未设置'}</SelectItem>)}</SelectContent></Select> : <textarea className='h-20 w-full resize-y overflow-hidden rounded border px-2 py-1 text-sm' value={value} onChange={(event) => onChange(event.target.value)} />}</div></div>
...
function Status({ value }: { value: string }) { return <span className='inline-flex max-w-20 truncate rounded-full bg-amber-50 px-1.5 py-0.5 text-[10px] text-amber-700'>{value === 'pending' ? '待搜索' : value}</span> }
